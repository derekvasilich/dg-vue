...
export const useVehicles = () => {
  const hasNextVehicle = computed(
    () => _currentIdx.value + 1 < filteredVehicles.value.length
  );
  const hasPreviousVehicle = computed(() => _currentIdx.value - 1 > -1);

  const _currentIdx = computed(() =>
    filteredVehicles.value.findIndex(
      (vehicle) => vehicle.node.id === currentVehicle.value.id
    )
  );

  const next = computed(() =>
    hasNextVehicle.value
      ? filteredVehicles.value[_currentIdx.value + 1].node
      : null
  );
  const prev = computed(() =>
    hasPreviousVehicle.value
      ? filteredVehicles.value[_currentIdx.value - 1].node
      : null
  );
  ...
  const filteredVehicles = computed(() => {
    if (
      typeof vehicles.value.map !== "function" ||
      typeof filters.value === "undefined"
    ) {
      return [];
    }
    let filtered = vehicles.value.filter((edge) => {
      ...
      if (
        typeof filters.value.query !== "undefined" &&
        vehicle.name.indexOf(filters.value.query) === -1
      ) {
        return false;
      }
      return true;
  ...
  const minMaxPrice = computed(() => {
    ...
    return vehicles.value.reduce(
      (acc, edge) => {
        ...
        !vehicle.price && (vehicle.price = 0);
        acc[0] =
          parseFloat(vehicle.price) < acc[0]
            ? parseFloat(vehicle.price)
            : acc[0];
        acc[1] =
          parseFloat(vehicle.price) > acc[1]
            ? parseFloat(vehicle.price)
            : acc[1];
        return acc;
      },
  ...
  const getVehicles = (opts = {}) => {
    ...
    vehicleService
      .getAll({ page: page.value, size: size.value })
      .then((res) => {
