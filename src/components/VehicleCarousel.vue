...
const images = computed(() => {
    ...
    if (props.currentVehicle?.description?.description?.style) {
        let styles = !isEmpty(props.currentVehicle?.description?.description?.style[0]) ? 
        props.currentVehicle?.description?.description?.style :
            [props.currentVehicle?.description?.description?.style];
        stockImages = styles.map((style) => { return { ...style.stockImage, alt: props.currentVehicle.name || `${style.division._} ${style.model._} ${style.name}` } })
    }
    return stockImages
})
</script>
