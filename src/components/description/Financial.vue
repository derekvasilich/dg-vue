...
const optionsPrice = computed(() => {
    return [].concat(factoryOption && factoryOption.value || []).reduce((total, option) => {
...
const vehiclePrice = computed(() => {
    const base = basePrice.value.invoice?.low || basePrice.value.msrp?.low || vehicle.price;
...
<template>
    <div>
        ...
        <ul>
            ...
            <dd v-if="basePrice.invoice?.low">{{ formatPriceRange(basePrice.invoice) }}</dd>
            
            <dt v-if="basePrice.destination">Destination</dt>
            <dd v-if="basePrice.destination">{{ formatPriceRange(basePrice.destination) }}</dd>

            <dt v-if="!isEmpty(selectedOptions)">Factory Options</dt>
            <dd v-if="!isEmpty(selectedOptions)">{{ formatPriceRange(optionsPrice) }}</dd>
        </ul>
    </div>
</template>